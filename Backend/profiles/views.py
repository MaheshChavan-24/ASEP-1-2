...
# ============================================================
# GEOSPATIAL PROXIMITY SYSTEM
# ============================================================

class FetchNearbyWorkersView(APIView):
    """
    POST/GET: Fetch nearby active workers within a 5.0 km radius.
    Expects client_lat and client_lon in the query parameters or request body.
    """
    # Temporarily allowing unauthenticated access for testing ease, 
    # but in production, we should uncomment this:
    # permission_classes = [permissions.IsAuthenticated]

    def get_coordinates(self, request):
        if request.method == 'GET':
            return request.query_params.get('client_lat'), request.query_params.get('client_lon')
        return request.data.get('client_lat'), request.data.get('client_lon')
    ...
    def request_handler(self, request):
        lat_str, lon_str = self.get_coordinates(request)

        if not lat_str or not lon_str:
            return Response({"error": "client_lat and client_lon are required."}, status=status.HTTP_400_BAD_REQUEST)

        try:
            client_lat = float(lat_str)
            client_lon = float(lon_str)
        except ValueError:
            return Response({"error": "Invalid coordinate format."}, status=status.HTTP_400_BAD_REQUEST)

        nearby_workers = []
        # Get all active workers with location
        active_workers = WorkerProfile.objects.filter(is_active=True).exclude(latitude__isnull=True).exclude(longitude__isnull=True)

        for worker in active_workers:
            distance = self.haversine_distance(client_lat, client_lon, worker.latitude, worker.longitude)
            if distance <= 5.0:
                serializer = WorkerProfileSerializer(worker)
                worker_data = serializer.data
                worker_data['distance_km'] = round(distance, 2)
                nearby_workers.append(worker_data)

        # Sort by closest distance
        nearby_workers.sort(key=lambda x: x['distance_km'])

        return Response({"count": len(nearby_workers), "workers": nearby_workers}, status=status.HTTP_200_OK)
